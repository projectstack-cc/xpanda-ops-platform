...
- [ ] **v2 logistics live-launch plan** — consolidates the remaining dark-gated items before this
  board can go live and get linked into nav: (1) flip the write fence above; (2) build the Status-field
  cascade — the new Shipment Edit Modal (`ShipmentEditModal.tsx`) intentionally ships Status as
  read-only, since making it editable requires porting legacy's job/`loading_assignments`/cutting-lines
  sync chain (`_worker.js/routes/jobs.js` shipment PUT handler), which doesn't exist in v2 yet; (3) add
  shipment delete + BOL-history-delete actions to the edit modal (legacy has both, v1 of the modal
  omits them as unnecessary risk on a still-fenced, still-unlinked board); (4) the nav cutover itself.
  All gated on Steve's explicit go-ahead per the v2 visibility gate hard rule.
...
- [ ] lb-ui-04 — saved loads: persist an edited `PackPlan` past the in-memory `editedPlan` state `lb-ui-02` introduced (lost today on fixture switch or page reload). Needs a D1 table + API routes — out of scope for every `lb-ui-NN` prompt so far ("no API routes, no D1, no persistence"). Sprint step 7 of 7, `Prompts/sprint-load-builder-parity.md`.
...
- [ ] **`lb-ui-05` follow-up — export `packEngine.ts`'s `colorForSku` instead of duplicating it.** `jobPull.ts`'s `colorForSkuId` is a byte-for-byte copy of `packEngine.ts`'s private, unexported `colorForSku` (same palette, same hash) so the job-pull preview's swatches match the eventual trailer-diagram colors. `packEngine.ts` is closed/ratchet-guarded so this prompt couldn't export it directly — worth doing in a future dedicated engine prompt to remove the duplicate.
...
- [ ] **`lb-ui-06` follow-up — auto-downsize and Force Sizes need new engine work before a UI can wire them.** Step 0 re-grepped `packEngine.ts` for `downsize`/`autoDownsize`/`forceSize`/`variant` — zero real matches (confirmed again at `lb-ui-06` time; `variant` only appears as a substring of "invariant"). Legacy's two remaining trailer-option toggles have no engine-side concept to bind to; scoping a `lb-engine-NN` prompt for auto-downsize/forced-size logic is a prerequisite, not a UI task.
