...
- [ ] lb-ui-02 — customize editor for the v2 load builder. Must reproduce legacy's (`logistics/load-builder.html`) customize guards, which v2 has no UI for yet: a piece-count-exceeded banner, an unassigned-pieces table, and the stack-height check — the only one of the three that's **blocking** (refuses the apply outright rather than just warning), and it must now check against **effective height** (`lb-engine-05`'s `dims.height - runnerHeight`), not nominal trailer height.
