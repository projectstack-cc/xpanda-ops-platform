...
- [ ] lb-engine-02 — joint orientation selection + width pairing + row assembly (implements `pack()`'s row/column geometry against `lb-engine-01`'s contracts).
- [ ] lb-engine-03 — column fill (K top-off against `topOffMinInchesPerPiece`), rear->front ordering, running balance, rationale strings (completes `pack()`).
