...
- [ ] lb-ui-03 — dissolve: moving columns across trailers via drag-and-drop, extending the holding area's already-trailer-agnostic pull/place across trailer boundaries (`loadEditor.ts`'s `moveColumn` already supports `from.t !== to.t` generically; only the UI wiring is scoped out today).
- [ ] lb-ui-04 — saved loads: persist an edited `PackPlan` past the in-memory `editedPlan` state `lb-ui-02` introduced (lost today on fixture switch or page reload). Needs a D1 table + API routes — out of scope for every `lb-ui-NN` prompt so far ("no API routes, no D1, no persistence").
