...
- [ ] **Per-module i18n extraction waves** — sweep in progress (2026-09-03). Phase 1 done: `shared/shared-header.js` (nav/notifications/sign-out/settings chrome on every module page) auto-loads the engine and is fully translated (load-order bug found + fixed same day). Phase 2 in progress, order: Logistics first, then remaining order doesn't matter (2026-09-04 direction). `logistics/index.html`, `loading.html`, `bol-email.html` done (`logistics/logistics-i18n.js`, 359 keys × en/es/ht). Still open in Logistics: `load-builder.html` (3054 lines) — deliberately deferred (2026-09-04): it's desk-side trailer-load planning, not a floor screen, and much of its text (SKU orientation-suffix labels baked into `sku.name`, `TRAILER_TYPES` object keys used as both lookup key and display label) is load-bearing inside the bin-packing algorithm and printed BOL/diagram output, so a full tag-everything pass isn't the right shape for it — plan is a narrower labels/buttons/forms/toasts/warnings-only pass, explicitly skipping the orientation-suffix labels and the `TRAILER_TYPES`/category keys (translating those requires decoupling display text from lookup keys, not attempted). `bol-test.html` (456, dev tool, no header shim) is lowest priority. Then: jobs, manufacturing, qc, production, reports (13 sub-pages, only `index.html` uses the shared header — the other 12 need their header/nav mechanism investigated first), admin (bespoke topbar, no `shared-header.js`, needs the i18n engine wired from scratch), `legal/*`/`login.html`/`track/index.html` (no header shim at all). Fast pass, no need for perfection — fix mistranslations later if flagged (2026-09-04 direction, supersedes any per-module native-speaker-review-item expectation). Only customer names are meant to stay untranslated — everything else, including dynamic/JS-generated content, is in scope.
- [ ] **Title/subtitle dead-code override bug on non-Logistics module pages** — found 2026-09-04 while surveying for the i18n sweep, fixed for `logistics/index.html` (deleted, duplicate of module default) and `logistics/bol-email.html` (load-bearing — added `layout.bolEmailTitle`/`bolEmailSubtitle` keys, switched to `window.I18n.t()`). Every legacy module index/sub-page sets its own `document.getElementById('<module>-page-title').textContent = '<hardcoded English>'` (and `-subtitle`) *after* `shared-header.js` already sets those elements correctly (and translated) from the `layout.*` catalog — silently overwriting the translation back to English. Still open: `jobs/index.html`, `jobs/packing-slip-test.html`, `logistics/load-builder.html`, `manufacturing/{index,block-calculator,holey-board-calculator}.html`, `production/{index,bead-inventory}.html`, `qc/{index,density-calculator,final-inspection,incident-report,scrap-log}.html`, `reports/index.html`. For index/dashboard pages the override just duplicates the `layout.*` default (safe to delete); for sub-pages with a distinct title (Block Calculator, Density Calculator, Load Builder, etc.) the override is load-bearing — those need new `layout.*` catalog keys and the inline JS switched to `window.I18n.t(...)` instead of a literal string. Fix as each page is reached in the per-module i18n sweep above, not as a separate pass.
