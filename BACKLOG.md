...
- [x] P261 hotfix #2 — even after the Drive/XLSX fix + working token, `schedule_rows` still stayed empty: `wrangler tail` caught the real cause, the scheduled handler was hitting the Workers CPU time limit parsing all 190+ historical tabs in the workbook by default. Fixed with SheetJS's `sheets` read option (parse only the 2 needed tabs, ~16s → ~5s) + `[limits] cpu_ms = 60_000` safety margin in `wrangler.toml`. Cron temporarily at `*/5 * * * *` to verify faster — **must revert to `*/15 * * * *` once a poll is confirmed writing rows.** **Awaiting redeploy + live confirmation.**
