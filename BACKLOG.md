...
- [ ] **Enable `/v2/logistics` writes** — flip `V2_LOGISTICS_WRITES_ENABLED` to `true` in `cutting-pilot/src/app/api/bols/route.ts` and `.../bols/[id]/route.ts` once Steve has reviewed the fenced POST/PUT logic live (dedicated write-enable phase, not bundled with unit 2's read-side rollout).
...
- [ ] **lb-engine-03 open question (B3) — does the floor crew load nose-first or rear-first?** `pack()` orders rows thickest-base at `posFromFront: 0` (the rear, where the doors are) on the assumption that the thickest freight is loaded last/unloaded first. If the floor actually loads nose-first instead, the on-screen diagram's row order and the pick list run in reverse of each other. Ask Steve; not acted on either way yet.
...
- [ ] lb-ui-01 — React plan view for the v2 load builder: trailer diagram + stack list, read-only (no save, no BOL generation yet), consuming `pack()`/`planMetrics()` from `packEngine.ts`. Also wires `packEngine.selfcheck.ts` into a dev-only surface so the 120-check suite is runnable from the browser instead of a throwaway `tsx` runner.
- [ ] **lb-ui-02 must reproduce legacy's customize guards.** `logistics/load-builder.html`'s customize editor enforces three checks v2 has no UI for yet: a piece-count-exceeded banner, an unassigned-pieces table, and the stack-height check — the only one of the three that's **blocking** (refuses the apply outright rather than just warning). All three need a v2 equivalent before the customize flow can replace legacy's.
...
- [ ] **Unit 3b follow-up — i18n for the new dock dashboard labels.** `DockAssignmentCard.tsx`/`AssignBayModal.tsx`/`LoadedChecklistModal.tsx`/`DockBoard.tsx` ship English-only strings (v2 has no i18n spine wired yet, matching every other v2 UI unit so far) — needs a pass once v2 gains one.
- [ ] **Unit 3b follow-up (low priority) — drag-and-drop for bay/queue moves.** Legacy supports mouse + touch drag between the awaiting queue and bay columns; the dock dashboard's port relies solely on the equivalent action buttons (Assign to bay / Move to yard / etc.), which cover every transition drag-and-drop does — this would only be a redundant, faster interaction path, not new capability.
- [ ] **Unit 3b follow-up — Shipping Info modal (job ship-to address detail).** Legacy's `openShippingInfo` (click an INV# to see full ship-to/contact/notes) wasn't ported; the dock dashboard shows customer + city/state on the card only.
