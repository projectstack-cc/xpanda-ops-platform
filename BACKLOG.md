...
- [ ] **Per-module i18n extraction waves** — sweep in progress (2026-09-03). Phase 1 done: `shared/shared-header.js` (nav/notifications/sign-out/settings chrome on every module page) now auto-loads the engine and is fully translated. Still open: each module's own page *content* — jobs, logistics, manufacturing, qc, production, reports (13 sub-pages, only `index.html` uses the shared header) each need their own `*-i18n.js` catalog registered against the shared engine and `data-i18n` tagging on their pages/dynamic JS. Admin (bespoke topbar, no `shared-header.js`) and `legal/*`/`login.html`/`track/index.html` (no header shim at all) need the same treatment plus their own header/chrome translated from scratch. Only customer names are meant to stay untranslated (2026-09-03 direction) — everything else, including dynamic/JS-generated content, is in scope.
