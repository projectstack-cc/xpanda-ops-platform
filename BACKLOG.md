...
- [ ] **Per-module i18n extraction waves** — sweep in progress (2026-09-03). Phase 1 done: `shared/shared-header.js` (nav/notifications/sign-out/settings chrome on every module page) auto-loads the engine and is fully translated (load-order bug found + fixed same day). Phase 2 in progress, order: Logistics first, then remaining order doesn't matter (2026-09-04 direction). `logistics/index.html` done (`logistics/logistics-i18n.js`, 221 keys × en/es/ht). Still open in Logistics: `load-builder.html` (3054 lines), `loading.html` (1551), `bol-email.html` (929) — all three already load `logistics-header.js` so they'll pick up the catalog automatically, just need `data-i18n` tagging + dynamic-JS routing; `bol-test.html` (456, dev tool, no header shim) is lowest priority. Then: jobs, manufacturing, qc, production, reports (13 sub-pages, only `index.html` uses the shared header — the other 12 need their header/nav mechanism investigated first), admin (bespoke topbar, no `shared-header.js`, needs the i18n engine wired from scratch), `legal/*`/`login.html`/`track/index.html` (no header shim at all). Fast pass, no need for perfection — fix mistranslations later if flagged (2026-09-04 direction, supersedes any per-module native-speaker-review-item expectation). Only customer names are meant to stay untranslated — everything else, including dynamic/JS-generated content, is in scope.
- [ ] **Title/subtitle dead-code override bug on non-Logistics module pages** — found 2026-09-04 while surveying for the i18n sweep, fixed only for `logistics/index.html` so far. Every legacy module index/sub-page sets its own `document.getElementById('<module>-page-title').textContent = '<hardcoded English>'` (and `-subtitle`) *after* `shared-header.js` already sets those elements correctly (and translated) from the `layout.*` catalog — silently overwriting the translation back to English. Confirmed present on: `jobs/index.html`, `jobs/packing-slip-test.html`, `logistics/bol-email.html`, `logistics/load-builder.html`, `manufacturing/{index,block-calculator,holey-board-calculator}.html`, `production/{index,bead-inventory}.html`, `qc/{index,density-calculator,final-inspection,incident-report,scrap-log}.html`, `reports/index.html`. For index/dashboard pages the override just duplicates the `layout.*` default (safe to delete, as done for `logistics/index.html`); for sub-pages with a distinct title (Block Calculator, Density Calculator, Load Builder, BOL Email Queue, etc.) the override is load-bearing — those need new `layout.*` catalog keys and the inline JS switched to `window.I18n.t(...)` instead of a literal string. Fix as each page is reached in the per-module i18n sweep above, not as a separate pass.
