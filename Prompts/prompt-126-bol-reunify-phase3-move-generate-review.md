# Prompt 126 — BOL re-unification, Phase 3 of 3: move generate + review into BolCompose
## Track context
Final phase of the BOL extraction. P125 moved the modal UI/state into `BolCompose` (generate/review stayed in `load-builder.html`, bridged via `ctx`). This moves **generate + save + review** into the module too, so `BolCompose` owns the whole BOL flow end-to-end. Load builder keeps only the load-builder-specific append pipeline (packing slip + loading diagram), now handed to the module as an `opts.buildAppendBytes(bm)` callback. The static `#bol-review-*` HTML moves into the module (injected). **Zero behavior change.**

## Agent setup
Read **both** `AGENTS.md` and `xpanda-ops-agents.md`. Assume the **logistics-agent**. No migration, no `_worker.js` change.

## DO NOT TOUCH
The auto-pack engine, `STORAGE_KEY` (`foam_trailer_loader_v31`), `bol-shared.js`, `bol-editor.js`, `bol-generator.html`, and the load-builder helpers `buildPackingList` / `buildLoadingDiagramPdfBytes` / `getDims` (they stay — `buildBolAppendBytes` calls them). The `state.bolModal` literal in `state = { … }` is now fully unused but harmless — leave it.

---

## Deliverable 1 — OVERWRITE `/logistics/bol-compose.js`
Replace the entire file (the P125 version) with this validated content. It now injects the review-modal markup, owns the review state, and contains `generateAll` / `generateBolPdf` (which delegates appended-PDF bytes to `OPTS.buildAppendBytes`) / `showReview` / `closeReview` / `editorOnApply` / `editorOnCancel` / `handleReviewEdit`:
...
### 2.1 — Add `buildBolAppendBytes(bm)` (the load-builder-specific append pipeline)
This is the body of the old `generateBolPdf` minus the final `BolShared.generatePdf` call — it gathers the packing slip and/or loading-diagram bytes and merges them, returning the combined bytes (or null). **INSERT it immediately before** `function openBolModal(result) {`:

```javascript
async function buildBolAppendBytes(bm) {
  ...
  if (bm && bm.includeLoadingDiagram) {
    // BolShared.generatePdf is called once for all BOLs (not per-trailer), so we
    // attach only the diagram for the currently-displayed trailer (currentPage).
    // When GENERATE ALL BOLs is triggered from the last page, this is the last trailer.
    try {
      const idx = bm.currentPage;
      const trailer = bm.trailers[idx];
      const trailerDims = trailer.downsizedDims || getDims();
  ...
  return combinedAppendBytes;
}
```

### 2.2 — Point the modal adapter at the new callback
FIND:
```javascript
...
REPLACE:
```javascript
    buildAppendBytes: buildBolAppendBytes,
```

### 2.3 — DELETE `generateAllBols` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
...
### 2.4 — DELETE `generateBolPdf (old version — fully replaced by buildBolAppendBytes above + the module's own generateBolPdf)` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
async function generateBolPdf(bolRecords, bm) {
  // Collect optional appended-PDF bytes (packing slip and/or loading diagram).
...
### 2.5 — DELETE `the lb review-state vars` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
let lbPendingBlobUrl    = null;
let lbPendingBols       = null;
...
### 2.6 — DELETE `showBolReviewLB` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
...
### 2.7 — DELETE `closeBolReviewLB` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
...
### 2.8 — DELETE `lbEditorOnApply` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
async function lbEditorOnApply(updatedBol) {
  lbReviewBols[lbReviewActiveIndex] = updatedBol;
  const editorHost = document.getElementById('bol-review-editor-host-lb');
  const iframe     = document.getElementById('bol-review-iframe');
  try {
...
### 2.9 — DELETE `lbEditorOnCancel` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
function lbEditorOnCancel() {
  document.getElementById('bol-review-editor-host-lb').style.display = 'none';
  document.getElementById('bol-review-iframe').style.display = '';
  document.getElementById('bol-review-picker-lb').style.display = 'none';
}
...
### 2.10 — DELETE `handleReviewEditLB` (moved into bol-compose.js)
Remove this entire block, leaving nothing:
```javascript
function handleReviewEditLB() {
  const iframe     = document.getElementById('bol-review-iframe');
  const editorHost = document.getElementById('bol-review-editor-host-lb');
  iframe.style.display     = 'none';
  editorHost.style.display = 'flex';
...
### 2.11 — DELETE the static BOL review modal HTML (moved into bol-compose.js, injected at load)
...
- In `load-builder.html`, grep returns ZERO for each: `generateAllBols`, `showBolReviewLB`, `closeBolReviewLB`, `lbEditorOnApply`, `lbEditorOnCancel`, `handleReviewEditLB`, `lbReviewBols`, `lbPendingBlobUrl`, `lbCtx`, `id="bol-review-backdrop"`, and `async function generateBolPdf`. The only new symbol is `buildBolAppendBytes`.
- Functional (must be identical to before): single-trailer load → GENERATE BOLs → fill → GENERATE ALL → progress → review modal opens → Make Changes → inline editor (BolEditor) → Approve → PDF opens + success view. Multi-trailer: pagination, carry-over, INV# fill, GENERATE ALL → review with the BOL picker (switch between BOLs) → Approve saves overrides for each. Toggle "Include packing slip" (with a job prefill) and "Include loading diagram" → appended pages appear in the preview exactly as before.
- Open/close the modal without generating — clean. Review modal Close/✕ — clean.

## Deploy
```
git add logistics/bol-compose.js logistics/load-builder.html
git commit -m "P126: BOL re-unification phase 3 — move generate/save/review into BolCompose; load-builder provides buildAppendBytes; review modal injected by module; zero behavior change"
git push
```
