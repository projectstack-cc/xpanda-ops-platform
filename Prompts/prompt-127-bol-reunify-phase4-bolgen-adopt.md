# Prompt 127 — BOL re-unification, Phase 4: bol-generator adopts the shared review surface
## Track context
Final phase of the BOL re-unification. P123–P126 extracted load-builder's BOL modal + generate + review into the self-contained `BolCompose` module (which injects its own review modal). `bol-generator.html` still has its **own** duplicate review modal + approve/edit handlers — and its review-modal HTML reuses the same element ids the module now injects, so the two can't coexist. This makes `bol-generator.html` consume the module's review surface instead, killing the duplication.

The module's review is **persistence-agnostic** via a new `BolCompose.reviewRecords(records, opts)` entrypoint: the module renders the preview, runs Make-Changes (BolEditor) + the multi-record picker, then on Approve calls `opts.onApprove(records)` (the caller persists) and opens the PDF. The authoring path (`open`/`render`/`generateAll`) is untouched.

`bol-generator.html` keeps everything that's unique to it — the form, recent-BOLs list, draft/save, duplicate, edit-existing (`loadBolIntoForm`), and the address manager. Only its bespoke review modal + handlers are removed.

## Agent setup
Read **both** `AGENTS.md` and `xpanda-ops-agents.md`. Assume the **logistics-agent**. No migration, no `_worker.js` change. Depends on P123–P126 already landed.

## DO NOT TOUCH
`bol-shared.js`, `bol-editor.js`, the auto-pack engine, `load-builder.html` (P125/P126 own it). In `bol-generator.html` leave intact: `collectPayload`, `validatePayload`, `handleGenerate`, `toggleHideDims`, `handleDraft`, `handleClear`, `handleDuplicate`, `loadBolIntoForm`, the address manager, customer/carrier search, recent-BOLs list.

## One intentional behavior change (note it)
The old approve persisted the form payload **without** `render_overrides` unless you clicked Make Changes first — so re-saving a loaded BOL that had overrides dropped them. The new `onApprove` persists `render_overrides` from the previewed record, so loaded overrides are **preserved** on a plain re-save. This is the correct P68 round-trip behavior; flag it in the commit.

---

## Deliverable 1 — OVERWRITE `/logistics/bol-compose.js`
Replace the entire file (the P126 version) with this validated content. Identical to P126 plus the new `reviewRecords` surface and its export:
...
## Deliverable 2 — edit `/logistics/bol-generator.html`
...
### 2.2 — route preview/review/approve through the module
FIND:
```javascript
    pendingPackingSlipBytes = packingSlipPdfBytes;
    const result = await BolShared.generatePdf([tempBol], { previewOnly: true, packingSlipPdfBytes });
    showBolReview(result.blobUrl, payload, tempBol);
```
REPLACE:
```javascript
    await BolCompose.reviewRecords([tempBol], {
      buildAppendBytes: () => packingSlipPdfBytes,
      onApprove: async (recs) => {
        const persistPayload = { ...payload, render_overrides: (recs[0] && recs[0]._overrides) || null };
        const isEdit = !!editingBolId;
        const res = isEdit
          ? await api.put(`/api/bols/${editingBolId}`, persistPayload)
          : await api.post('/api/bols', persistPayload);
        if (!res.ok) throw new Error(res.error || 'Save failed.');
        const bolLabel = res.data.bol.bol_number ? `BOL #${res.data.bol.bol_number}` : 'BOL';
        showToast(`${bolLabel} ${isEdit ? 'updated' : 'generated'}`);
        loadRecentBols();
      },
      onError: (e) => setStatus('Error: ' + (e.message || String(e)), 'error'),
    });
```

### 2.3 — DELETE the now-unused review state vars
Remove this block, leaving nothing:
```javascript
let pendingReviewPayload    = null;
let pendingReviewBlobUrl    = null;
let pendingReviewBol        = null;
let pendingPackingSlipBytes = null;
```

### 2.4 — DELETE the bespoke review handlers (moved to / replaced by the module)
Remove this entire block (`showBolReview`, `closeBolReview`, `handleReviewApprove`, `handleReviewEdit`), leaving nothing:
```javascript
function showBolReview(blobUrl, payload, bolForEditor) {
  pendingReviewPayload = payload;
  pendingReviewBlobUrl = blobUrl;
  pendingReviewBol     = bolForEditor || null;
...
async function handleReviewApprove() {
  ...
  try {
    const isEdit = !!editingBolId;
    const res = isEdit
      ? await api.put(`/api/bols/${editingBolId}`, pendingReviewPayload)
      : await api.post('/api/bols', pendingReviewPayload);
    const data = res.data;
    if (!res.ok) {
...
function handleReviewEdit() {
  if (!pendingReviewBol) { closeBolReview(); return; }
  const iframe     = document.getElementById('bol-review-iframe');
  const editorHost = document.getElementById('bol-editor-host');
  iframe.style.display     = 'none';
  editorHost.style.display = 'flex';
  BolEditor.open(pendingReviewBol, editorHost, {
    onApply: async function (updatedBol) {
      try {
        editorHost.style.display = 'none';
        iframe.style.display     = '';
        iframe.src = '';
        pendingReviewPayload.render_overrides = updatedBol._overrides || null;
        const result = await BolShared.generatePdf([updatedBol], {
          previewOnly: true,
          packingSlipPdfBytes: pendingPackingSlipBytes,
        });
        if (pendingReviewBlobUrl) URL.revokeObjectURL(pendingReviewBlobUrl);
        pendingReviewBlobUrl = result.blobUrl;
        iframe.src = result.blobUrl;
      } catch (e) {
        setStatus('Error regenerating preview: ' + (e.message || String(e)), 'error');
        closeBolReview();
      }
    },
    onCancel: function () {
...
### 2.5 — DELETE the bespoke review modal HTML (the module injects its own)
Remove this entire block near the end of `<body>`, leaving nothing:
```html
<!-- BOL Review Modal -->
<div id="bol-review-backdrop" style="
  position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:10000;
...
">
  ...
  ">
    ...
    <iframe id="bol-review-iframe" style="
      flex:1;border:none;border-radius:0 0 12px 12px;
    "></iframe>
    <div id="bol-editor-host" style="flex:1;display:none;flex-direction:column;overflow:hidden;border-radius:0 0 12px 12px;"></div>
  </div>
</div>
```
...
- In `bol-generator.html`, grep returns ZERO for each: `showBolReview`, `closeBolReview`, `handleReviewApprove`, `handleReviewEdit`, `pendingReviewPayload`, `pendingPackingSlipBytes`, `id="bol-review-backdrop"`, `bol-editor-host`.
- New-BOL flow: fill the form → Generate BOL PDF → review modal (module's) → Make Changes → inline editor → back → Approve & Save → PDF opens, toast fires, recent list refreshes, BOL saved (POST).
- Edit flow: open a recent BOL (`loadBolIntoForm`, edit banner shows) → Generate → Approve → saves via PUT to the same id; overrides preserved.
- Duplicate flow: duplicate → BOL number cleared → Generate → Approve → saves as new (POST).
- Draft, Clear, address manager, customer/carrier search, hide-dims, packing-slip include toggle (with a job prefill) — all unchanged.

## Deploy
```
git add logistics/bol-compose.js logistics/bol-generator.html
git commit -m "P127: BOL re-unification phase 4 — bol-generator adopts BolCompose.reviewRecords (shared review surface); removes duplicate review modal/handlers; preserves overrides on re-save"
git push
```

## Tail (not this prompt)
With both consumers on the shared surface, giving `bol-generator.html` true **multi-trailer** is now small: collect N ship-to sets and pass `reviewRecords([rec1, rec2, …])` — the multi-record picker already handles navigation. Scope when you want it.
