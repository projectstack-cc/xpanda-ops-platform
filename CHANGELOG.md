...
- **P192** — Auto-populate ship date next to shipper signature: `shipperDate` coord added (`x: 107, y: 48, size: 10`); after the cursive shipper-signature block, `_displayDate` is drawn at that coord via the regular `drawText` helper. Reuses the already-computed `_displayDate` so the signature-area date always agrees with the top-right date and respects any `date` override. Renders on all copies (default/driver/customer). Frontend-only, `logistics/bol-shared.js`.
