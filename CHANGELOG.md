...
- **P324** — Block nesting engine + cut-sheet renderer, completing `/v2/blocks`. New
  ...
  rectangles of height `tlo+thi+0.25"` (odd qty tracked as one scrap complement wedge — a genuine
  extra unit of that size, per Steve: set aside for a future order or eventually scrapped, not
  necessarily waste). Two chunk-building tiers, terminology matched to the reference spreadsheet's
  own "Method & Assumptions" sheet: **Strict** (`buildChunksStrict`) groups by exact native width,
  height-FFD per group, no cross-SKU reuse — the naive floor. **Greedy** (`buildChunksGreedy`, the
  prompt's required baseline) processes rectangles widest-first; a narrower rectangle may be
  admitted into an *existing* chunk's remaining height with its width trimmed down to exactly that
  chunk's established width (`trimmedFrom` flagged only when strictly narrower); a chunk's length
  is the max native length among its residents, so a shorter resident just leaves an unrecovered
  "length end" gap (reported per-line via `NestChunkLine.partWidth`/`partLength`, not re-packed).
  Both tiers get FFD-packed into fixed-length molds (`(n-1) × 0.25/3"` charged per block for
  internal cross-cuts), and **the smaller-or-equal block count wins by explicit comparison** —
  "never regress below the strict floor" is a hard guarantee by construction. **Scoped limitation
  (matches the reference spreadsheet's own stated scope — it hasn't built this either):** the
  block-level width-strip and the per-band length-end are computed/displayed but never re-pooled
  as additional packer capacity — would need full 2D/3D guillotine bin-packing. New
  `NestChunkLine`/`NestChunk`/`NestBlock`/`DensityNestResult`/`NestResult` types in `blockTypes.ts`.
  New `src/app/blocks/CutSheet.tsx` (per density → per mold → per chunk: table beside a diagram)
  and `ChunkElevation.tsx` (SVG side elevation scaled to the real block face, aspect-correct;
  bands alternate taper-diagonal lean band-to-band; a trimmed or naturally-shorter band shades its
  gap and labels it "→ pool"). `BlocksApp.tsx` now calls `nest(...)` from **Reload cut sheet** and
  renders `<CutSheet>` in place of the P323 stub. **Verified exactly against the real PO#1
  spreadsheet** (same `po1Fixture.ts` as P323, all 57 real line items, blocks sized 50×50×198):
  **1# = 10 molds / floor 6.31 / 0 scrap wedges, 1.5# = 3 / 2.03 / 0, 2# = 3 / 2.09 / 3** — all
  three densities, every number, exact match, no tolerance needed beyond floating-point rounding.
  (The naive Strict tier alone independently reproduces the sheet's separate "Strict molds"
  row — 12/5/5 — confirming both tiers are implemented correctly, not just the required one.) The
  dev self-check (`blockNester.selfcheck.ts`) asserts this real baseline directly, plus the
  structural invariants it already covered (reconciliation, scrap-wedge counting,
  `blocksNeeded >= ceil(volumeFloor)`, `greedy <= strict` including a constructed case proving the
  lever strictly wins). `tsc --noEmit` + `cf-build` green.
- **P323** — Block nesting input half (`/v2/blocks`, P322 scaffold): `src/lib/blockTypes.ts` (shared
  ...
  that P324 will swap for the real nester. **Verified against the real PO#1 spreadsheet**
  (`xPanda_PO1_Nesting_Map.xlsx`, provided by Steve 2026-08-04, extracted into
  `src/lib/po1Fixture.ts` — committed, no customer PII/credentials, real SKU codes/dims/qty only):
  all 57 real raw descriptions parse with **zero mismatches** against the sheet's own decoded
  columns, and the full parse+combine pipeline reproduces the exact manager-hand-checked baseline
  for every density — **1# = 18 SKUs/302 pcs, 1.5# = 13/74, 2# = 16/59**. The dev self-check
  (`src/lib/poParser.selfcheck.ts`, guarded to non-production, logged from a `BlocksApp` effect)
  now asserts this real baseline directly, in addition to the hand-built messy-variant/combine/
  unparsed-row fixtures it already covered. `tsc --noEmit` + `cf-build` green.
...
- **P429** — `/v2/orders` "From parts library" picker: new `PartsPicker.tsx` composing the shared `<Modal>`, searchable by part #/customer, fetching the legacy `/api/parts` directly (shared cookie). Selecting a part appends a prefilled line item and carries `part_id` through to the order route (`OrderLineItem` gained optional `part_id`; submit payload includes it). No schema/route change. `tsc --noEmit` + `cf-build` green.
...
- **P369 — Schedule status now distinguishes actively-cutting from started-then-stopped.**
  ...
  `in_progress` with no open session → **"In Production – X%"** (started, then Stopped, not yet
  complete), X = checked `cutting_line_progress` rows ÷ (`job_line_items` count × `cutting_lines`
  count) for the job, floored and clamped 0–100, `null` (plain "In Production") when the
  denominator is 0. Return type changed from `Map<string, ScheduleStatus>` to
  `Map<string, DerivedStatus>` (`{ status, progressPct }`); `schedule-board/route.ts` threads the
  new `progress_pct` field through `ScheduleBoardRow`, mirrored in `types/schedule.ts`.
  `StatusBadge.tsx` renders the percentage suffix only for "In Production" and only when
  `progressPct` is non-null. No dedicated production/progress token family exists in
  `globals.css`/`shared/tokens.css`, so the new badge reuses the `--accent`/`--accent-soft` pair
  (soft bg + accent text/border) — visually distinct from Cutting's blue `--info-*` and Ready's
  amber `--warn-*` in both themes. No schema change, no new endpoint.
- **P365** — Fixed the auto-hide header covering the board on pointer reveal (`/v2/schedule` only — the mini PC driving the wall TV has a mouse attached). `PlatformHeader`'s reveal state (`revealed`) is local `useState` with no exposed signal — it's a shared component used by every `/v2/*` page, so per this prompt's hard boundary it was left untouched rather than adding a prop to it. Instead `ScheduleBoard.tsx` implements a schedule-scoped reveal observer: the same `pointermove`/`keydown`/`touchstart` triggers on `window` and the same 5s idle timeout as `PlatformHeader`'s private `NAV_AUTO_HIDE_IDLE_MS` (duplicated as a documented local constant — kept in sync manually since there's no exposed signal to read it from), driving local `headerRevealed` state. Header height is read live off the DOM (`rootRef.current.querySelector("header")` + `ResizeObserver`, scoped to the schedule subtree only) rather than hardcoded. All three render branches (loading, error, loaded) now render a `shrink-0` spacer div between `PlatformHeader` and the board content, animating its `height` with the same `transition-[height] duration-300 ease-out` (+ `motion-reduce:transition-none`) as the header's own `translate-y` reveal, so the board reflows down in sync instead of being overlaid, and reclaims the space on hide. No pointer input (the TV) never fires the listeners, so the spacer stays `height: 0` and the board renders full-bleed exactly as before. `PlatformHeader.tsx` and every other `/v2/*` route are untouched. `tsc --noEmit` + `cf-build` green.
...
- **P389** — Added 10 missing DiversiTech H-series parts (`H2448-4` … `H4558-4`) surfaced by the
  ...
  claimed, none of the 10 part numbers already exist (no collision, guard is a safety net for
  re-runs only), and the 5 existing H-series rows all carry `allow_rotation=0`/`bundle_qty=0` —
  consistent with the new rows' unspecified-column defaults, so no extra columns needed adding.
...
- **P387 — packing-slip matcher reads standalone trailing HB thickness; fee lines excluded
  ...
  dimension/part-number passes, the alias table. **Deviated from the prompt's own "Bilateral: …
  legacy only" claim** — grepped `cutting-pilot/src` (same bilateral-miss pattern P386 just fixed
  for a sibling file) and found a third, unmentioned verbatim port: `cutting-pilot/src/lib/
  packingSlip.ts`'s `extractThickness()` (wired live into v2 order entry via `OrderEntryForm.tsx`)
  had the identical `[xX×]\s*`-required bug and the same missing fee-line filter in its finalizer.
  Applied both fixes there too so all three surfaces agree; `npx tsc --noEmit` green after. `node
  --check` clean on the legacy parser; all anchors verified flipped (old strings → 0, new strings →
  1) before and after.
