...
import { getValidToken, saveConnection, fetchInvoice, fetchInvoiceByDocNumber } from '../lib/quickbooks.js';
...
export async function handleApiQuickbooks(request, env) {
  ...
  if (request.method === 'POST' && subRoute === 'import') {
    ...
    // Duplicate guard (mirrors POST /api/jobs)
    ...
    for (const li of mapped.line_items) {
      ...
      `).bind(
        ...
        null, li.part_number, li.description, li.quantity, '', li.sort_order,
      ).run();
    }

    // Auto-create outbound shipment (mirrors POST /api/jobs)
    try {
      await db.prepare(`
      ...
      console.error('QB import: auto-shipment creation failed:', String(e?.message || e));
    }

    // Auto-create loading assignment (mirrors POST /api/jobs)
    try {
      await db.prepare(`
      ...
      console.error('QB import: auto-loading-assignment creation failed:', String(e?.message || e));
    }
    ...
    return json({ ok: true, job_id: id, invoice_number: mapped.invoice_number, customer: mapped.customer }, 201);
  }
